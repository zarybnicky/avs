...
int main(int argc, char *argv[])
{
    ...
    options.add_options()
      ...
      ("b,builder", "Builder name [ref, loop, tree, cached]", cxxopts::value<std::string>()->default_value("ref"))
    ...
    try
    {
        ...
        else if(args["builder"].as<std::string>() == "cached")
        {
            CachedMeshBuilder builder(args["grid"].as<unsigned>());
            builder.setBatchMode(args.count("batch"));
            builder.buildMesh(field, args["output"].as<std::string>());
        }
        else
